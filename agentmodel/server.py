...
def compute_color(agent):
    if agent.is_mod:
        ...
        return "#CC0000" # red
    elif agent.trolling_received == 0:
        return "#037f51" # green
    elif agent.trolling_received < 5:
...
def network_portrayal(G):
    ...
    portrayal['nodes'] = [{'id': node_id,
                           ...
                           'label': None if not agents else 'Agent:{} Trolling:{}'.format(agents[0].unique_id,
                                                                                        agents[0].trolling_received),
                           }
...
grid = NetworkModule(network_portrayal, 500, 500, library='d3')
chart = ChartModule([
    {"Label": "Average Trolling", "Color": "Black"}],
